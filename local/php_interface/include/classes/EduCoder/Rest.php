...
use \EduCoder\Exceptions\RestUnknownResponse;
use \EduCoder\Exceptions\RestClientError;
use \EduCoder\Exceptions\RestUnauthorized;
use \EduCoder\Exceptions\RestForbidden;
use \EduCoder\Exceptions\RestMethodNotAllowed;
use \EduCoder\Exceptions\RestBadRequest;
use \EduCoder\Exceptions\RestNotFound;
use \EduCoder\Exceptions\RestConflict;
use \EduCoder\Exceptions\RestGone;
use \EduCoder\Exceptions\RestInvalidRecord;
use \EduCoder\Exceptions\RestServerError;
use \EduCoder\Exceptions\RestCurlInit;
use \EduCoder\Exceptions\RestCurlMeta;
use \EduCoder\Exceptions\RestCurlExec;
...
class Rest
{
    ...
    protected function prepRequest($opts, $url) {
        ...
        $curl = curl_init($url);
        if ($curl === false) {
            throw new RestCurlInit($this->processError(curl_error($curl), 'curl'));
        }
    ...
    protected function checkLastResponseForError() {
        ...
        switch ($meta['http_code']) {
            case 400:
                throw new RestBadRequest($this->processError($body));
                break;
            case 401:
                throw new RestUnauthorized($this->processError($body));
                break;
            case 403:
                throw new RestForbidden($this->processError($body));
                break;
            case 404:
                throw new RestNotFound($this->processError($body));
                break;
            case 405:
                throw new RestMethodNotAllowed($this->processError($body));
                break;
            case 409:
                throw new RestConflict($this->processError($body));
                break;
            case 410:
                throw new RestGone($this->processError($body));
                break;
            case 422:
                ...
                throw new RestInvalidRecord($this->processError($body));
                break;
            default:
                if ($meta['http_code'] >= 400 && $meta['http_code'] <= 499) {
                    throw new RestClientError($this->processError($body));
                } elseif ($meta['http_code'] >= 500 && $meta['http_code'] <= 599) {
                    throw new RestServerError($this->processError($body));
                } elseif (!isset($meta['http_code']) || $meta['http_code'] >= 600) {
                    throw new RestUnknownResponse($this->processError($body));
                }
        }
    }
